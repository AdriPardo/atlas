...
4. **Hoy (consola web):** `POST /projects/{id}/database/console-session` + botón **Open database** → ticket one-shot `/db-console/connect/{id}` (pgweb Connect Backend; Authentik). Sin password en query string. Ver [db-console-hostname.md](../deployment/db-console-hostname.md).
