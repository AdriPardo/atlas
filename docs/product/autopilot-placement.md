...
4. Confirm **Deploy** (no host picker required).
5. Land on the deployment detail / logs.
...
```text
Connect app → Deploy(exposure?)
        │
        ├─ Host selection (this slice)
        │     Prefer LOCAL + online
        │     Else hostname atlas-local / default
        │     Else any LOCAL → any online → first host
        │     Else create default LOCAL host (atlas-local)
        │
        ├─ Exposure
...
|----------|------|--------|
| **Reuse shared host** | Default for thin apps / same fleet | **This slice** — auto-pick LOCAL / default host |
| **Provision new VM** | Isolation, resource floor, or no capacity on shared hosts | **Next slice** — Proxmox provisioner (API not in this PR) |
...
|----------------|----------------------|
| **Host** | Placement target (LOCAL socket or SSH). Still created/synced; usually auto-selected or auto-seeded. |
...
Nothing in the Host/Deploy/Jobs stack is throwaway. Future Proxmox VMs become new Host rows that the same deploy job already knows how to drive.
