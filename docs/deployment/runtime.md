...
Autopilot ISOLATED (Proxmox): `ATLAS_PROXMOX_API_URL` / `NODE` / `TEMPLATE_VMID` + secrets `proxmox.api.token` y `proxmox.ssh.private_key`. Reuse on por defecto (`ATLAS_PROXMOX_REUSE_ENABLED=true`): Host/VM por hostname o tag Proxmox antes de clonar. Clone off por defecto (`ATLAS_PROXMOX_CLONE_ENABLED=false`); con clone on, Atlas espera guest-agent (o `ATLAS_PROXMOX_DEFAULT_GUEST_IP`). Ver [ADR-0012](../decisions/ADR-0012-autopilot-proxmox-provisioner.md).
